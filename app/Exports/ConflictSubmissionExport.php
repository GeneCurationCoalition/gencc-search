    ...
    public function __construct(Collection $groups, string $format)
    {
        ...
        $this->rows = $groups->flatMap(function ($group) {
            ...
            foreach ($submissions as $index => $submission) {
                $indexed[] = ['index' => $index, 'submission' => $submission];
            }
            ...
            usort($indexed, function ($a, $b) {
                ...
                return $leftSide <=> $rightSide
                    ...
                    ?: strcasecmp((string) $left['submitter_title'], (string) $right['submitter_title'])
                    ?: strcmp((string) $left['sgc_id'], (string) $right['sgc_id'])
                    ?: $a['index'] <=> $b['index'];
            });

            return array_column($indexed, 'submission');
        })->values();
    }
